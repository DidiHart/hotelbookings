...
//returns true if availability exists for roomId, false if no availabilty exists
...
//returns slice of available rooms, if any, for a given date range
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	row := m.DB.QueryRowContext(ctx, query, email) //get at most one row from the query
